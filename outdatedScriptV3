...
Other
    - Leaked by 77fuscator developers
    - I, User319183 was working on the deobfuscator for 77fuscator but I didn't really give a shit since it got leaked
...
if getgenv().Logging == nil then
	getgenv().Logging = false
end
...
if getgenv().RunTime == nil then
	getgenv().RunTime = 0
end

if getgenv().CollectAirdrops == nil then
	getgenv().CollectAirdrops = false
end

if getgenv().KillAura == nil then
	getgenv().KillAura = false
end

if getgenv().RobSS == nil then
	getgenv().RobSS = false
end

if getgenv().AutoOpen == nil then
...
local ui_options = {
	main_color = Color3.fromRGB(202, 97, 130),
...
local function TeleporterC(x,y,z, duration)
	local tper = game:GetService("RunService").Heartbeat:Connect(function()
		vehicleRoot.CFrame = CFrame.new(x,y,z)
	end)
...
local robberies = {
	bank = {open = false, hasRobbed = false},
	jewerly = {open = false, hasRobbed = false},
	tomb = {open = false, hasRobbed = false},
	museum = {open = false, hasRobbed = false},
	airdrop = {open = false, hasRobbed = false},
	ship = {open = false, hasRobbed = false},
	ctrain = {open = false, hasRobbed = false},
	plane = {open = false, hasRobbed = false},
	ptrain = {open = false, hasRobbed = false},
	gasstation = {open = false, hasRobbed = false},
	donutstore = {open = false, hasRobbed = false},
}
...
coroutine.wrap(LPH_JIT_MAX(function()
	while true do
		UpdateStatus(robberyConsts.ENUM_ROBBERY.BANK, 'bank', 'open')
		UpdateStatus(robberyConsts.ENUM_ROBBERY.JEWELRY, 'jewerly', 'open')
		UpdateStatus(robberyConsts.ENUM_ROBBERY.TOMB, 'tomb', 'open', true, false)
		UpdateStatus(robberyConsts.ENUM_ROBBERY.MUSEUM, 'museum', 'open', true, false)
		robberies['airdrop'].open = (game.Workspace:FindFirstChild('Drop') and true) or false
		UpdateStatus(robberyConsts.ENUM_ROBBERY.CARGO_SHIP, 'ship', 'open')
		UpdateStatus(robberyConsts.ENUM_ROBBERY.TRAIN_CARGO, 'ctrain', 'open')
		UpdateStatus(robberyConsts.ENUM_ROBBERY.CARGO_PLANE, 'plane', 'open')
		UpdateStatus(robberyConsts.ENUM_ROBBERY.TRAIN_PASSENGER, 'ptrain', 'open')
		UpdateStatus(robberyConsts.ENUM_ROBBERY.STORE_GAS, 'gasstation', 'open')
		UpdateStatus(robberyConsts.ENUM_ROBBERY.STORE_DONUT, 'donutstore', 'open')
		task.wait()
	end
end))()
...
function GetRobbingJewel()
	local Wall = workspace.Jewelrys:GetChildren()[1].Wall.SecurityWall
...
function BagFull()
	...
	return (Moneys[1] == Moneys[2]) or tonumber(Moneys[1]:match("%d+")) >= (999999)
end
...
local InVehicle = function() return ((vehicle and vehicle.Name == 'Jeep' or 'Camaro' or 'Heli') and true) or false end
...
local function SmallTP(cframe, speed, v)
...
local function CheckRaycast(Position, Vector)
	local Raycasted = game.Workspace:Raycast(Position, Vector, tele)
...
local function EnterCar()
	...
	if Packet() and table.find(OwnedHelis, GetVehicleModel().Name) then
		HideCar()
		return true
	else
		ExitVehicle()
		repeat task.wait() until not Packet()
	end

	local SortedCars = game.Workspace:WaitForChild("Vehicles"):GetChildren()
	...
	for _, v in pairs(SortedCars) do        
		if v.PrimaryPart ~= nil and v.Seat.PlayerName.Value == "" and table.find(OwnedHelis, v.Name) and not CheckRaycast(v.PrimaryPart.Position, Vector3.new(0, 1000, 0)) then
			FlightMove(v.Seat.CFrame + Vector3.new(0, 3, 0))
			GoToGround()
			local Timeout = tick()
			repeat
				...
				task.wait(0.3)
			until Packet() or tick() - Timeout > 5
...
do 
	local Window = library:AddWindow("Jailbreak", {
		main_color = Color3.fromRGB(202, 97, 130),
	...
	local Tab1 = Window:AddTab("Auto rob")
	local Tab2 = Window:AddTab("Store Status")
	local Tab3 = Window:AddTab("Safes")
	local Tab4 = Window:AddTab("Misc")

	do
		local StatusLabel = Tab1:AddLabel("Autorob Disabled")

		local AutoRobEnabled = Tab1:AddSwitch("Auto Rob", function(a)

		end)

		local KillAuraEnabled = Tab1:AddSwitch("Kill Aura", function(a)

		end)

		local AirdropsEnabled = Tab1:AddSwitch("Collect Airdrops", function(a)

		end)

		local RobSS = Tab1:AddSwitch("Rob Small Stores", function(a)

		end)

		Tab1:AddButton("Copy Discord Invite", function(a)
			setclipboard("https://discord.gg/tempcode")
		...
		local MoneyEarnedLabel = Tab1:AddLabel("Money Earned: $0")
		local ElapsedTimeLabel = Tab1:AddLabel("Elapsed Time: 0h/0m")

		local BankStatus = Tab2:AddLabel("Bank")
		local JewerlyStatus = Tab2:AddLabel("Jewerly")
		local TombStatus = Tab2:AddLabel("Tomb")
		local MuseumStatus = Tab2:AddLabel("Museum")
		local AirdropStatus = Tab2:AddLabel("Airdrop")
		local CargoShipStatus = Tab2:AddLabel("Cargo Ship")
		local CargoTrainStatus = Tab2:AddLabel("Cargo Train")
		local CargoPlaneStatus = Tab2:AddLabel("Cargo Plane")
		local PassengerTrainStatus = Tab2:AddLabel("Passenger Train")
		local GasStationStatus = Tab2:AddLabel("Gas Station")
		local DonutStoreStatus = Tab2:AddLabel("Donut Store")

		local AutoOpenSafes = Tab3:AddSwitch("Auto Open", function(a)
			local yes = a
			print(yes)
			getgenv().AutoOpen = yes
		end)

		local OpenAll = Tab3:AddButton("Open All Safes", function(a)
		...
		Tab4:AddLabel("Coming soon ;)")

		library:FormatWindows()

		local function RobJSTORE()
			...
			if robberyState[tostring(robberyConsts.ENUM_ROBBERY.JEWELRY)].Value == 2 then
				TeleporterC(137, 19, 1337, 1.5)
			else
				TeleporterC(137, 19, 1337, 1.5)
				
				task.wait(.1)
				
				if not SmallTP(CFrame.new(136, 19, 1350), nil, 2000) then return end	
				if not SmallTP(CFrame.new(135, 19, 1337), nil, 2000) then return end
			end
		end

		task.spawn(function()
			while task.wait() do
				if robberies.bank.open then BankStatus.TextColor3 = Color3.new(0,1,0) else BankStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.jewerly.open then JewerlyStatus.TextColor3 = Color3.new(0,1,0) else JewerlyStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.tomb.open then TombStatus.TextColor3 = Color3.new(0,1,0) else TombStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.museum.open then MuseumStatus.TextColor3 = Color3.new(0,1,0) else MuseumStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.airdrop.open then AirdropStatus.TextColor3 = Color3.new(0,1,0) else AirdropStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.ship.open then CargoShipStatus.TextColor3 = Color3.new(0,1,0) else CargoShipStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.ctrain.open then CargoTrainStatus.TextColor3 = Color3.new(0,1,0) else CargoTrainStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.plane.open then CargoPlaneStatus.TextColor3 = Color3.new(0,1,0) else CargoPlaneStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.ptrain.open then PassengerTrainStatus.TextColor3 = Color3.new(0,1,0) else PassengerTrainStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.gasstation.open then GasStationStatus.TextColor3 = Color3.new(0,1,0) else GasStationStatus.TextColor3 = Color3.new(1,0,0) end
				if robberies.donutstore.open then DonutStoreStatus.TextColor3 = Color3.new(0,1,0) else DonutStoreStatus.TextColor3 = Color3.new(1,0,0) end
			end
		end)
		...
		task.spawn(function()
			while true do
				...
				if SafeAmt ~= 0 and getgenv().AutoOpen then
					for i = 1, SafeAmt do
						local CurrentSafe = Modules.Store._state.safesInventoryItems[1]
						...
						if not getgenv().AutoOpen then break end
					end
				end
